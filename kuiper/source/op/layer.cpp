...
void Layer::set_input(int32_t idx, const tensor::Tensor& input) {
  CHECK_GE(idx, 0); // check if idx is greater than or equal to 0
  CHECK_LT(idx, inputs_.size()); // check if idx is less than the size of inputs_
  this->inputs_.at(idx) = input; // 将传入的 input 张量赋值给 inputs_ 向量中索引为 idx 的位置，使用 at() 方法而不是 [] 操作符，因为 at() 会进行边界检查
}
